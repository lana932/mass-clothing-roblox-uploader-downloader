...
# If user doesn't have one of the modules, it will tell them to install all of the required
    ...
    print("[ERROR] Failed to import some modules, make sure to run requirements.bat and setup.exe, delete all other python versions and install python 3.10.0 installed with add to path option checked during installation")
    input()
...
try:
    ...
    print(f"{Back.RED}{Fore.BLACK}[Error]{Back.BLACK}{Fore.WHITE} Your cookie is invalid run setup.exe, and paste your cookie into config.ini")
